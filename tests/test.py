...
import parallelDG.graph.junction_tree_expander as jtexp
import parallelDG.graph.junction_tree as libj
import parallelDG.graph.graph as glib
import parallelDG.graph.junction_tree_expander
import parallelDG.graph.junction_tree_collapser as jtcol
import parallelDG.graph.subtree_sampler
...
def expand(tree, node, alpha, beta, directory=None):
    ...
    (subtree, subtree_nodes, subtree_edges, subtree_adjlist,
     old_separators, prob_subtree) = parallelDG.graph.subtree_sampler.random_subtree(old_tree, alpha, beta, (node, node))
    (old_cliques,
    new_cliques,
    new_separators,
    P,
    neig) = parallelDG.graph.junction_tree_expander.sample_cond_on_subtree_nodes(node, tree, subtree_nodes, subtree_edges, subtree_adjlist)

    K_st = parallelDG.graph.junction_tree_expander.pdf(old_tree, tree, alpha, beta, node)
    return K_st
        ...
        if np.diag(adjmat).sum() == 0:
            ...
            if nx.is_chordal(g):
                if check_symmetric(adjmat):
                    # print libg.mu(g)
                    graphs[adjmatvec] = parallelDG.graph.decomposable.n_junction_trees(g)
                    jt = parallelDG.graph.decomposable.junction_tree(g)
                    S = jt.get_separators()
