...
import parallelDG.graph.trajectory as mc
...
from parallelDG.distributions import multivariate_students_t as tdist
from parallelDG.distributions import sequential_junction_tree_distributions as seqjtdist
...
#        classes = list(set(np.array(y).flatten()))
    ...
    def gen_gibbs_chains(self, n_particles, n_pgibbs_samples, smc_radius=None, cta_alpha=0.5, cta_beta=0.5, async=True):
        ...
        for g, group in enumerate(self.same_graph_groups):
            ...
            if async is True:
                async_results[g] = pool.apply_async(parallelDG.pgibbs.sample_trajectory,
                                                    (n_particles, cta_alpha,
                                                     cta_beta,
                                                     self.smc_radius, n_pgibbs_samples,
                                                     seq_dist))
            else:
                self.ggm_trajs[g] = parallelDG.pgibbs.sample_trajectory(n_particles, cta_alpha,
                                                                      cta_beta,
                                                                      self.smc_radius,
                                                                      n_pgibbs_samples,
                                                                      seq_dist)
