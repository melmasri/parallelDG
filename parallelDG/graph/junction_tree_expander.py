...
import parallelDG
import parallelDG.graph.subtree_sampler as ss
...
def sample(tree, i, alpha=0.5, beta=0.5, only_tree=True):
    ...
    #old_G = parallelDG.graph.junction_tree.get_graph(tree)
    ...
    #G = parallelDG.graph.junction_tree.get_graph(tree)
    # G.node[i] = {"color": "red"}
    # for n in old_G:
    #     if n in conn_nodes:
...
def subtree_cond_pdf(tree1, tree2, tree2_subtree_nodes, new):
    ...
    if len(tree2_subtree_nodes) == 1 and list(tree2_subtree_nodes.values())[0] is None:
        sep = frozenset([])
        c = frozenset([new])
        lognu = parallelDG.graph.junction_tree.log_nu(tree2, sep)
...
def sample_cond_on_subtree_nodes(new, tree, subtree_nodes, subtree_edges, subtree_adjlist):
    ...
    if subtree_order == 0:
        ...
        sep = frozenset()
        #tree.fix_graph()
        parallelDG.graph.junction_tree.randomize_at_sep(tree, sep)
...
def get_subtree_nodes(T1, T2, new):
    ...
    T2_ind = parallelDG.graph.junction_tree.subtree_induced_by_subset(T2, {new})
